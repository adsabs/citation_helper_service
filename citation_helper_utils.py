...
def get_suggestions(**args):
    ...
    Nsuggestions = config.NUMBER_SUGGESTIONS
    # get rid of potential trailing spaces
    bibcodes = map(lambda a: a.strip(), bibcodes)[:config.MAX_INPUT]
    # start processing
    ...
    paperFreq = filter(lambda a: a[1] > config.THRESHOLD_FREQUENCY, paperFreq)
